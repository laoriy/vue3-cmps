...
export function stubExistingAdditionalProperties(
    ...
    Object.keys(formData).forEach((key) => {
        ...
        (schema as any).properties[key] = additionalProperties(
            ...
            schema as any
        ).properties[key][ADDITIONAL_PROPERTY_FLAG] = true;
    });
...
function withDependentSchema(
    ...
    const resolvedOneOf = oneOf.map((subschema) => {
        const oneOfSchema = hasOwnProperty(subschema, '$ref')
...
export function mergeSchemas(obj1: any, obj2: any) {
    ...
    return Object.keys(obj2).reduce((accs, key) => {
        const left = obj1 ? obj1[key] : {};
        const right = obj2[key];
        if (obj1 && hasOwnProperty(obj1, key) && isObject(right)) {
            accs[key] = mergeSchemas(left, right);
        } else if (
            obj1 &&
            obj2 &&
...
export function getSchemaType(schema: Schema): string | undefined {
    ...
    if (t instanceof Array && t.length === 2 && t.includes('null')) {
        return t.find((atype) => atype !== 'null');
    }
...
export function isConstant(schema: Schema) {
    return (
        (Array.isArray(schema.enum) && schema.enum.length === 1) || schema.hasOwnProperty('const')
    );
}
...
export function isSelect(_schema: any, rootSchema: Schema = {}) {
    ...
    if (Array.isArray(altSchemas)) {
        return altSchemas.every((altSchemass) => isConstant(altSchemass));
    }
    return false;
}
...
export function mergeDefaultsWithFormData(defaults: any, formData: any): any {
    ...
    if (isObject(formData)) {
        ...
        return Object.keys(formData).reduce((accs, key) => {
            accs[key] = mergeDefaultsWithFormData(defaults ? defaults[key] : {}, formData[key]);
            return accs;
        }, acc);
    }
    return formData;
